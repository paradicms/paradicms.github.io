...
The [Paradicms work search app](https://paradicms.org/docs/reference/apps/#work-search) provides a faceted and fulltext search interface over the set of [works](https://paradicms.org/docs/introduction/data-model/) in a digital collection. The app can be configured to facetize, filter, and search arbitrary domain-specific imageProperties of works, such as the condition of a garment.
...
* Filters for various user-defined imageProperties are shown in an inline sidebar, with an accordion that ensures that only one filter is visible at a time.
