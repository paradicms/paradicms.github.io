...
A Paradicms-formatted spreadsheet such as the <GoogleSheetsTemplateLink>Google Sheets template</GoogleSheetsTemplateLink> consists of multiple sheets. Each sheet corresponds to a class in the [Paradicms ontology](/docs/reference/ontology), such as `Person` and `Work`, and is named after that class, as in the Google Sheets screenshot below:
