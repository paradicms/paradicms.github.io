...
The example is adapted from the <GoogleSheetsGitHubTemplateRepositoryLink rawPath="/.github/workflows/build.yml">GitHub Actions workflow in the Google Sheets template repository</GoogleSheetsGitHubTemplateRepositoryLink>. The [tutorial on using Google Sheets to generate a faceted search interface](/docs/tutorials/spreadsheet-ssg) explains the role of this action in context.
