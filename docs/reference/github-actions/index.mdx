...
The `directory-etl-action` extracts, transforms, and loads collection data from a directory of files. The files must reside in the GitHub repository.
...
The example is adapted from the [directory template repository](https://raw.githubusercontent.com/minorg/ComputerScienceInventions/main/.github/workflows/build.yml).
...
```yaml
uses: paradicms/markdown-etl-action@v1-beta
```
